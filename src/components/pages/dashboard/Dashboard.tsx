...
export const Dashboard = () => {
  ...
  const handleAddLink = () => {
    if (newLink.trim() && newQuote.trim()) {
      setLinks((prev) => [...prev, { link: newLink, quote: newQuote }]);
      setNewLink('');
      setNewQuote('');
    }
  };
  ...
  const handleLinkQuoteChange = (index: number, value: string) => {
    const updated = [...links];
  ...
  const handleLinksSubmit = async () => {
    ...
    await fetch('http://localhost:8000/uploadlink', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
